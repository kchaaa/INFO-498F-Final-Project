...
safe_first_draw <- data %>% 
                   ...
                   group_by(Ward) %>% summarise(First_Ward_Num=sum(Ward==Ward))
...
safe_45sec_num <- safe_45_sec %>% 
                  group_by(Ward) %>% 
                  summarise(Sec_Ward_Num=sum(Ward==Ward))
...
safe_2_min <- data %>% 
              ...
              group_by(Ward) %>% 
              summarise(Min_Ward_Num=sum(Ward==Ward))
...
warning_first_num <- warning_first_draw %>% 
                     group_by(Ward) %>% 
                     summarise(First_Ward_Num=sum(Ward==Ward))
...
above_num  <- left_join(above_min, above_2min_num)
...
## Introduction 
  On March 6, 2016, the 2016 Democratic Presidential Candidates, Bernie Sanders and Hillary Clinton, held a debate in Flint, Michigan. Holding a democratic debate in Flint highlights the dire problem that is happening in this city known as the Flint Water Crisis. 
  The Flint Water Crisis is about how the residents of Flint have been suffering from lead-infected water. Before 2013, the city of Flint obtained water from Detroit. In April 2013, the Flint city council voted to join the Karegnondi Water Authority (KWA) that promised that they would get water from Lake Huron. This deal will save the city $19 million over eight years instead of continuing to pay Detroit for water. In response to Flint's decision, Detroit announced that they would stop providing Flint water in April 2014 even though the KWA won't be ready for another 3 years. Despite previous concerns about the conditions of the river, the emergency manager in Flint decided that they should use the Flint River as a backup water source. After using it, residents begin to notice and complain about the poor tasting and smelly water. The city found fecal coliform bacteria and recommended residents boil their water and buy bottled water. The government of Michigan and Flint worked to try to fix the water source; however, the state government refused to allow Flint to switch their water supply back to Detroit. Due to the government's effort, the city finally was able to improve the quality fo their water. Their celebration would be short because scientists soon found lead in the water due to old pipes and poor infrastructure. The State government promised to use $1 million to buy water filters for the city of Flint and finally allowed Flint to use Detroit water again. 
  The lead-infected water has serious health effects on the residents of Flint especially the children and pregnant women of Flint. It can affect growth their growth, behaviors, cognition, and hearing. Consuming lead-infected water can impact the residents' heart, kidneys, and nerves. There are medications that would reduce the amount of lead in their blood to reduce the negative effects of lead; however, no medication exist to treat for the adverse health effects. The government recommended they boil water as a way to clean their water, but boiling water results in higher lead concentration because a heavy metal like lead doesn't evaporate like water does. 

## Analysis (Change the Title)
  A few months ago, a test was done in Flint. Three tests were conducted using the residents' toilets: 1) the first draw, 2) 45 seconds after flushing, and 3) 2 minutes after flushing. Then, they tested the amount of lead in each sample in parts per billion (ppb). If the sample is below 5 ppb, the water is fine. If the sample is between 5 ppb and 15 ppb, then the water is relatively safe, but it is on the cusp of the danger. If the sample is above 15 ppb,  then the households are at high risk and the EPA and the government needs to take action because 15 ppb is the allowed EPA level. We can observe and compare the results of each test:
